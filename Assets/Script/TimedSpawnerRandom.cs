...
public class TimedSpawnerRandom: MonoBehaviour {
    ...
    [Tooltip("Maximum distance in X between spawner and spawned objects, in meters")] [SerializeField] float distance = 0.5f;
    ...
    // async Task SpawnRoutine() {  // async-await
        while (true) {
            ...
            int scale = (int)Random.Range(-1, +2);
            Debug.Log(scale);
            Vector3 positionOfSpawnedObject = new Vector3(
                transform.position.x + scale * distance,
